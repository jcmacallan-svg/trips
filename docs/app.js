...
const DATASETS = {
  ww1: "./data/ww1-belgium.yaml",
  berlin: "./data/berlin-trip.yaml",
};
...
function render(doc, window.__STATE){
  const regions = doc.regions || [];
  const pois = doc.pois || [];
  ...
  // Apply filters
  const q = normalize(window.__STATE.search || "");
  const regionFilter = window.__STATE.region || "";
  const typeFilter = window.__STATE.type || "";
  const themeFilter = window.__STATE.theme || "";
  const photosOnly = !!window.__STATE.photosOnly;
